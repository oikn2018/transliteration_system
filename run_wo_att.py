...
# Load Best Model without Attention for 'ben' and 'hin'
...
def train():
    ...
    train_iterator, val_iterator, test_iterator = BucketIterator.splits(
        ...
        sort_within_batch=True,  # Examples of similar length will be in the same batch to minimize padding
    ...
    # Set word and original transliteration based on the configured Indic language
    if indic_lang == 'hin':
        word = 'bachta'
        og_translit = 'बचता'
    elif indic_lang == 'ben':
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in loop:
            ...
            output = output[1:].reshape(-1, output.shape[2])  # Reshape output for loss calculation
            target = target[1:].reshape(-1)
            optimizer.zero_grad()
            loss = criterion(output, target)

            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1)  # Clip gradients to avoid exploding gradients
            optimizer.step()
